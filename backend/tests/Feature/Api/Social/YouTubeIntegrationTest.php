 ...
 *
 * Tests for YouTube OAuth flow and video upload API endpoints:
 * - OAuth authorization URL generation
 * - OAuth callback handling
 * - OAuth code exchange
 * - OAuth account connection
 * - Video upload endpoint integration
 *
...
beforeEach(function () {
    // Use fake adapter factory to avoid real HTTP calls to YouTube OAuth
...
/**
 * Helper: get YouTube authorization URL and return the state parameter.
 */
function getYouTubeOAuthState(\Illuminate\Foundation\Testing\TestCase $test): string
{
    $response = $test->getJson('/api/v1/oauth/youtube/authorize');
 ...
 */
function exchangeYouTubeCode(\Illuminate\Foundation\Testing\TestCase $test, ?string $state = null): string
{
    $state ??= getYouTubeOAuthState($test);
    $response = $test->postJson('/api/v1/oauth/youtube/exchange', [
        'code' => 'test_youtube_authorization_code',
        'state' => $state,
    ]);
    $response->assertOk();
...
describe('YouTube OAuth Authorization', function () {
    it('returns authorization URL for YouTube', function () {
        Sanctum::actingAs($this->admin);

        $response = $this->getJson('/api/v1/oauth/youtube/authorize');
    ...
    it('includes YouTube-specific OAuth scopes', function () {
        Sanctum::actingAs($this->admin);
        ...
        $url = $response->json('data.url');
        // YouTube uses Google OAuth with specific scopes
        expect($url)->toContain('scope');
    });

    it('stores state in cache with YouTube platform', function () {
        Sanctum::actingAs($this->admin);
        ...
        expect($cachedData['platform'])->toBe('youtube');
    });
...
describe('YouTube OAuth Callback', function () {
    it('redirects to frontend with code and state for YouTube', function () {
        $response = $this->get('/api/v1/oauth/youtube/callback?' . http_build_query([
            'code' => 'test_youtube_authorization_code',
        ...
        expect($location)->toContain('code=test_youtube_authorization_code');
    ...
    it('redirects with error on OAuth provider error', function () {
        $response = $this->get('/api/v1/oauth/youtube/callback?' . http_build_query([
            'error' => 'access_denied',
            'error_description' => 'User denied YouTube access',
        ]));

        $response->assertRedirect();
        $location = $response->headers->get('Location');
        expect($location)->toContain('error=access_denied');
        expect($location)->toContain('User+denied+YouTube+access');
    });
...
describe('YouTube OAuth Code Exchange', function () {
    it('exchanges code for session key and YouTube channel info', function () {
        Sanctum::actingAs($this->admin);

        $state = getYouTubeOAuthState($this);

        $response = $this->postJson('/api/v1/oauth/youtube/exchange', [
            'code' => 'test_youtube_authorization_code',
            'state' => $state,
        ]);
    ...
    it('returns YouTube channel information', function () {
        Sanctum::actingAs($this->admin);
    ...
    it('validates state matches YouTube platform', function () {
        Sanctum::actingAs($this->admin);

        // Get state for LinkedIn
        $linkedinResponse = $this->getJson('/api/v1/oauth/linkedin/authorize');
        $linkedinState = $linkedinResponse->json('data.state');

        // Try to use LinkedIn state for YouTube exchange
        $response = $this->postJson('/api/v1/oauth/youtube/exchange', [
            'code' => 'test_youtube_authorization_code',
            'state' => $linkedinState,
        ]);
    ...
    it('state can only be used once', function () {
        ...
        $response1 = $this->postJson('/api/v1/oauth/youtube/exchange', [
            'code' => 'test_youtube_authorization_code',
            'state' => $state,
        ]);
        $response1->assertOk();
        ...
        $response2 = $this->postJson('/api/v1/oauth/youtube/exchange', [
            'code' => 'test_youtube_authorization_code',
            'state' => $state,
        ]);
...
describe('YouTube OAuth Account Connection', function () {
    it('connects YouTube channel to workspace via session key', function () {
        Sanctum::actingAs($this->admin);

        $sessionKey = exchangeYouTubeCode($this);

        $response = $this->postJson('/api/v1/oauth/youtube/connect', [
        ...
        expect($account->platform->value)->toBe('youtube');
    });

    it('stores YouTube channel metadata', function () {
        Sanctum::actingAs($this->admin);
    ...
    it('denies editor from connecting YouTube via OAuth', function () {
    ...
    it('session key can only be used once', function () {
        ...
        $response1 = $this->postJson('/api/v1/oauth/youtube/connect', [
            'workspace_id' => $this->workspace->id,
            'session_key' => $sessionKey,
        ]);
        ...
        $response2 = $this->postJson('/api/v1/oauth/youtube/connect', [
            'workspace_id' => $this->workspace->id,
            'session_key' => $sessionKey,
        ]);
    ...
    it('allows owner to connect YouTube channel', function () {
...
describe('YouTube OAuth State Security', function () {
...
describe('YouTube Video Upload Endpoint', function () {
    it('requires authenticated YouTube account for video operations', function () {
        Sanctum::actingAs($this->admin);

        // First connect a YouTube account
        $sessionKey = exchangeYouTubeCode($this);
    ...
    it('stores refresh token for long-term access', function () {
        ...
        // YouTube provides refresh tokens for offline access
